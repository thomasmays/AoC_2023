...
input_file = r"C:\Users\thomas.mays\OneDrive - Arup\Python Projects\Advent of Code 2023\11\sample_input.csv"
...
for src_galaxy_num, src_galaxy_loc in galaxy_locs.items():
    ...
    for dest_galaxy_num, dest_galaxy_loc in dest_galaxies.items():
        ...
        for i in range(row_steps):
            path.append((src_galaxy_loc[0]+i, src_galaxy_loc[1]))
        for j in range(col_steps):
            path.append((src_galaxy_loc[0]+i, src_galaxy_loc[1]+j))

        path_length = 0
        for step in path:
            value = image.iloc[step]
            if value == '.' or value == '#':
                path_length = path_length + 1
            else:
                path_length = path_length + value

        path_lengths[str(src_galaxy_num) + " & " + str(dest_galaxy_num)] = path_length
